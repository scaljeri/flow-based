...
/** The websocket's drawing, for datagrams — state, count, and where to. */
@Component({
  standalone: true,
  selector: 'fb-webtransport-small',
  template: `
    <div class="top">
    ...
    <span class="count" [class.error]="!!worker?.error">
      {{worker?.error ?? (worker?.received ?? 0) + ' datagrams'}}
    </span>
...
export class WebTransportSmallComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.worker = this.service.worker as WebTransportWorker | undefined;
